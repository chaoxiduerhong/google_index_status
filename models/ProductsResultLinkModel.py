...
class ProductsResultLinkModel(BaseModel):
    """
    任务状态status：
    ...
    def __init__(self):
        ...
        # 表名称，子类必须重写该表名称
        self.table_name = gpt_conf.product_table_result_urls
    ...
    def getFirstProductByFetchLink(self, link_version, bid=None, end_time=None, ext_condition=None):
        """
        异步出队列
        ...
        if not result:
            condition1 = {'link_processing': "waiting"}
        ...
        if not result:
            ...
            condition1 = {'time': {"$lt": end_time}, 'link_version': {"$lt": link_version}}
            condition = {**condition1, **ext_condition}
            result = self.first(
                condition=condition
            )
        ...
        if not result:
            condition1 = {'time': {"$lt": end_time}, 'link_version': {"$ne": link_version}}
            condition = {**condition1, **ext_condition}
            result = self.first(
                condition=condition
            )
